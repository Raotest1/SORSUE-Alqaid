...
API_ID = int("14105431")
API_HASH = "e8e1fda9c3b479087aa650a1f3c4566a"
Bots = []
Musi = []
CASER = [] 
off =None
...
async def me(client, message):
   if off:
    if not message.from_user.username in DEVS and not message.from_user.username in DEVSs:
     return await message.reply_text("**المجاني مغلق من قبل المبرمج @l3l3M**")
   try:
...
async def alivehi(client: Client, message):
    if message.from_user.username in CASER:
        return
    keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("مجموعة السورس", url=f"{group}"), InlineKeyboardButton("قناه السورس ", url=f"{source}")]])
    ...
    for i in bots:
        try:
            ...
            for group in get_groups(bot_id):
             ...
             l += 1
    return await ask.reply_text(f" تم الحظر في {g} مجموعة ✨♥")
    ...
    for i in bots:
        try:
            ...
            for group in get_groups(bot_id):
             ...
             l += 1             
    return await ask.reply_text(f" تم  الغاء الحظر في {g} مجموعة ✨♥")
