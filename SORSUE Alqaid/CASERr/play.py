...
async def change_stream(bot_username, chat_id, client): 
    ...
    if chat_id in playlist and playlist[chat_id] and vidd and vidd[chat_id] and namecha and namecha[chat_id] and user_mentio and user_mentio[chat_id] and thu and thu[chat_id] and phot and phot[chat_id]:
        ...
        try:
            ...
            await apppp.send_message(logger, f"**╭── : [sᴏᴜʀᴄᴇ ᴄʀᴀʏғᴏʀᴅ]({soesh}) : ──╮\n\n⌁ |𝗣𝗹𝗔𝘆𝗜𝗻𝗚 𝗡𝗼𝗪 𝗦𝘁𝗔𝗿𝗧𝗲𝗗\n\n⌁ |𝗦𝗼𝗡𝗴 𝗡𝗮𝗠𝗲 : `{thum}`\n⌁ |𝗕𝘆 : {user_mention}\n⌁ |𝗚𝗿𝗢𝘂𝗣 𝗕𝘆 : [{namechat}]({loggerlink})\n\n╰── : [sᴏᴜʀᴄᴇ ᴄʀᴀʏғᴏʀᴅ]({soesh}) : ──╯**", disable_web_page_preview=True)
        except Exception as e:
            pass
    else:
        try:
...
async def mson5674hfbg(client, message):
        ...
        try: 
          ...
          await h.edit_text("تم التحديث بنجاح ✅")
        except Exception as e:
...
async def msonhfbg(client, message):
    ...
    if message.reply_to_message:
     ...
     button = [[InlineKeyboardButton(text="𝗘𝗡𝗗", callback_data=f"stop"), InlineKeyboardButton(text="𝗥𝗘𝗦𝗨𝗠𝗘", callback_data=f"resume"), InlineKeyboardButton(text="𝗣𝗘𝗨𝗦𝗘", callback_data=f"pause")], [InlineKeyboardButton(text="𝗖𝗛𝗔𝗡𝗘𝗟", url=f"{soesh}"), InlineKeyboardButton(text="𝐆𝐑𝐎𝐔𝐏", url=f"{gr}")], [InlineKeyboardButton(text=f"{name}", url=f"https://t.me/{CASER}")], [InlineKeyboardButton(text="𝗔𝗱𝗗 𝗕𝗼𝗧 𝗧𝗼 𝗬𝗼𝗨𝗿 𝗚𝗿𝗢𝘂𝗣", url=f"https://t.me/{bot_username}?startgroup=True")]]
     ...
     await client.send_message(logger, f"**╭── : [sᴏᴜʀᴄᴇ ᴄʀᴀʏғᴏʀᴅ]({soesh}) : ──╮\n\n⌁ |𝗣𝗹𝗔𝘆𝗜𝗻𝗚 𝗡𝗼𝗪 𝗦𝘁𝗔𝗿𝗧𝗲𝗗\n\n⌁ |𝗦𝗼𝗡𝗴 𝗡𝗮𝗠𝗲 : `{thum}`\n⌁ |𝗕𝘆 : {user_mention}\n⌁ |𝗚𝗿𝗢𝘂𝗣 𝗕𝘆 : [{namechat}]({loggerlink})\n\n╰── : [sᴏᴜʀᴄᴇ ᴄʀᴀʏғᴏʀᴅ]({soesh}) : ──╯**", disable_web_page_preview=True)
    elif message.text:
     try:
      text = message.text.split(None, 1)[1]
     except Exception as e:
      nme = await client.ask(message.chat.id, text="**شتريد تشغل حبيبي**", reply_to_message_id=message.id, filters=filters.user(message.from_user.id), timeout=200)
      text = nme.text
    else:
        return
    if text in yoro:
      return await message.reply_text("**ماشغل شي حرام**")  
    else:      
     print("احم")    
    mm = await message.reply_text("**جاري التشغيل...**")    
    try:
...
async def msonhfbhdhjhg(client, message):
    ...
    if message.reply_to_message:
     ...
     try:
      ...
      nme = await client.ask(message.chat.id, text="**شتريد تشغل حبيبي**", reply_to_message_id=message.id, timeout=200)
      text = nme.text
    else:
        return
    if text in yoro:
...
async def gers(client, message):
    ...
    if group_id in thu:
        ...
        response = f"**╭── : [sᴏᴜʀᴄᴇ ᴄʀᴀʏғᴏʀᴅ]({soesh}) : ──╮\n\n⌁|𝗧𝗵𝗘 𝗦𝗼𝗡𝗴𝗦 𝗢𝗻 𝗧𝗵𝗘 𝗟𝗶𝗦𝘁:\n\n⌁|𝗡𝘂𝗠𝗯𝗘𝗿 𝗦𝗼𝗡𝗴𝗦: {count}\n\n**"
        if count == 0:
            return await message.reply_text("**لا يوجد شي في القائمة**")
        else:
        ...
        await message.reply_text("**لا يوجد شي في القائمة**")
...
async def admin_risghts(client: Client, CallbackQuery):
    ...
    if command == "pause":
        try:
         ...
         await CallbackQuery.answer("لا يوجد شي شغال الان", show_alert=True)
         await CallbackQuery.message.reply_text(f"**لا يوجد شي شغال الان {CallbackQuery.from_user.mention}**")
    if command == "resume":
        try:
         ...
         await CallbackQuery.answer("لا يوجد شي شغال الان", show_alert=True)
         await CallbackQuery.message.reply_text(f"**لا يوجد شي شغال الان يا {CallbackQuery.from_user.mention}**")
    if command == "stop":
       try:    	
...
async def ghuser(client, message):
    ...
    if chek.status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR] or message.from_user.username in caes:
     ...
     try:    	
      ...
      await ho.edit_text("** تم أيقاف التشغيل . **")
     except Exception as e:
      await ho.edit_text("**لا يوجد شي شغال الان**")    
    else:
...
async def sp2(client, message):
    ...
    if chek.status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR] or message.from_user.username in caes:
     chat_id = message.chat.id
     ho = await message.reply_text("**تم ايقاف التشغيل .**") 
     try:    	
      await hoss.pause_stream(chat_id)
      await ho.edit_text("**تم ايقاف التشغيل .**")
     except Exception as e:
      await ho.edit_text("**لا يوجد شي شغال الان**")
    else:
     return await message.reply_text(f"**عذرا عزيزي{message.from_user.mention}\n هذا الامر لا يخصك✨♥**")
...
async def s12p2(client, message):
    ...
    if chek.status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR] or message.from_user.username in caes:
     chat_id = message.chat.id
     ho = await message.reply_text("**حسناً ، سوف أكمل التشغيل .**") 
     try:    	
      await hoss.resume_stream(chat_id)
      await ho.edit_text("**تم التشغيل**")
     except Exception as e:
      await ho.edit_text("**لا يوجد شي شغال الان**")
    else:
     return await message.reply_text(f"**عذرا عزيزي{message.from_user.mention}\n هذا الامر لا يخصك✨♥**")
