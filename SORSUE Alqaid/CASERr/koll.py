...
async def ghsdh_user(client, message):
    ...
    hh = await message.reply("اصبر اطلع اشوف منو في الكول") 
    try:
     ...
     await message.reply(f"حبيبي الكول مو مفتوح اصلاً\n😜")
...
async def vc(c, message):
    ...
    if (group_call := await get_group_call(user, message, err_message="الكول مفتوح")):
     await message.reply_text("الكول مفتوح اصلاً")
     return        
    try:
     ...
     await hh.edit_text(f"قم برفع الحساب المساعد مشرف في المجموعة")
...
async def end_vc(c, message):
    ...
    if not (group_call := await get_group_call(user, message, err_message="الكول مقفول")):
     await hh.edit_text("الكول مقفول اصلاً")
     return        
    try:
