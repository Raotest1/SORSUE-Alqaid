...
async def gigshkdnnj(client, message):
    ...
    if await johned(client, message):
     return
    if message.chat.id in alkl:
      return await message.reply_text(f"عذرا عزيزي [{message.from_user.mention}] الامر معطل من قبل مالك المجموعة")
    keybord = InlineKeyboardMarkup([[InlineKeyboardButton("الحماية", callback_data=f"jzhfjgh5")]])
    chat_idd = message.chat.id
    chat_name = message.chat.title
    chat_username = f"@{message.chat.username}" if f"@{message.chat.username}" else "لا يوجود"   
    await message.reply_text(f"الإعــدادات\n\n¦ لمجموعة: {chat_name}\n¦ ايدي المجموعة: -{chat_idd}\n¦ معرف المجموعة: {chat_username}\n\nاضغط على الحمايه بالاسفل", reply_markup=keybord)
...
async def tit5le(client, message):
    ...
    if user.status in [ChatMemberStatus.OWNER]:
        await message.reply_text("مالك المجموعة")
...
async def caesarprivileges(client, message):
    ...
    if status == ChatMemberStatus.OWNER:
        await message.reply_text("أنت مالك المجموعة")
        ...
        hossam += f"تغيير معلومات المجموعة: {can_change_info}\n"
...
async def rotpty(client, message):
    ...
    if message.from_user.username in caes:
        ...
        await message.reply_text(f"**انت مشرف في المجموعة **")
