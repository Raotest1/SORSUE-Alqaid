...
Keyboard = ReplyKeyboardMarkup(
  [
    ...
    [("تعيين مجموعة السورس"), ("تعيين قناه السورس")],   
    ...
    [("المجموعةات"), ("المستخدمين")],
    [("رفع نسخه المجموعةات"), ("رفع نسخه الاشخاص")],
    [("قسم الترويج")],
  ],
  resize_keyboard=True
)
...
async def bot_name(client, message):
  ...
  if message.from_user.id == OWNER_ID or message.from_user.username in caes:
   try:
    bot = await client.ask(message.chat.id, "حسناً قم بإرسال أسم البوت الجديد", timeout=200)
   except:
    return
   bot_name = bot.text
...
async def dev_user(client, message):
  ...
  if message.from_user.username in caes or message.from_user.username == dev:
   try:
    bot = await client.ask(message.chat.id, "حسناً قم بإرسال معرف المطور", timeout=200)
   except:
    return
...
async def dev_channel(client, message):
  ...
  if message.from_user.username in caes or message.from_user.username == dev:
   try:
    bot = await client.ask(message.chat.id, "حسناً قم بإرسال معرف القناة", timeout=200)
   except:
    return
   bot_name = bot.text
...
@Client.on_message(filters.regex("تعيين مجموعة السورس") & filters.private, group=711513)
async def dev_group(client, message):
  ...
  if message.from_user.username in caes or message.from_user.username == dev:
   try:
    bot = await client.ask(message.chat.id, "حسناً قم بإرسال رابط المجموعة", timeout=200)
   except:
    return
   bot_name = bot.text
   devgroup[bot_username] = bot_name
   await client.send_message(message.chat.id, "**تم تعيين مجموعة السورس بنجاح ✨♥**")
  else:
   await message.reply_text(f"مرحبا عزيزي {message.from_user.mention} هذا الأمر في الوضع المدفوع، تواصل مع مطور السورس")
...
async def johned(client, message):
   ...
   for x in get_channel(bot_username):
    ...
    try:
      ...
      return await message.reply_text(f"‹ : عذراً عزيزي ، {message.from_user.mention} يجب عليك الاشترك في القناة اولا..\n\n    قنـاة الـبـوت :\n ⤹ https://t.me/{ch} ⤸", disable_web_page_preview=True, reply_markup=keyboard)
 
caesar_responses = [
    "وياك حبيبي",
    "ترا البوت شغال كابتن ؟",
    "ها شتريد",
    "ولك المسودن كلي شتريد",
    "ها ياعيون {name}",
    "عيون {name} انت",
    "هم كلك واحد قبل انت هواي تسولف",
    "ولك والله موتتني ماتكلي شكو كلشوية تصيحني",
    "كول كلبي",
    "بربك لو بمكاني ترضى واحد يلح عليك هيج ؟",
]
...
bot = [
    ...
    "احبك ابني",
    "ولك يصيحلي {name}",
    "والله العظيم اسمي {name}",
    "تريدني اشغلك حزين حتى تنسا ماضيك البائس ؟",
    "اللهم صلي على محمد وال محمد",
    "ولك ضايج اني وخر مني",
    "عيوني منو مزعلك ؟",
    "منو مضوجك اموته الك والله ؟",
    "المسودن",
    "بعد مرور فترة من الزمن سوف يصبح {name} عاجز بحكم انهُ مجرد ذكاء اصطناعي وانساك وتنساني",
]
...
async def for_us65ers(client, message):
   ...
   button = [[InlineKeyboardButton(text="‹ عـربـي ›", callback_data=f"arbk"), InlineKeyboardButton(text="‹ English ›", callback_data=f"english")],[InlineKeyboardButton(text=f"{namew}", url=f"https://t.me/{wenru}")], [InlineKeyboardButton(text="𝗔𝗱𝗗 𝗕𝗼𝗧 𝗧𝗼 𝗬𝗼𝗨𝗿 𝗚𝗿𝗢𝘂𝗣 ⤶", url=f"https://t.me/{bot_username}?startgroup=True")]]
   ...
   if await check(message.from_user.id, bot_username, bot_id):
     ...
     return await message.reply_photo(photo=photo, caption=f"**.       ╭─── : [sᴏᴜʀᴄᴇ ᴄʀᴀʏғᴏʀᴅ]({soesh}) : ───╮\n\n⌁ | مرحباً بك {message.from_user.mention}\n⌁ | انا {botmention}\n⌁ | اتمتع بأفضل أداء في سرعه التشغيل\n⌁ | واجمل حمايه مجموعات وقنوات\n⌁ | واقوى منع تصفيه\n⌁ | مع جميع الرتب من الادمن للاساسي\n⌁ | ضفني في المجموعة وتاكد بنفسك\n\n        ╰─── : [sᴏᴜʀᴄᴇ ᴄʀᴀʏғᴏʀᴅ]({soesh}) : ───╯**", reply_markup=InlineKeyboardMarkup(button))
   kep = ReplyKeyboardMarkup([["مطور البوت"], ["🥺 ¦ حذف الكيبورد"]], resize_keyboard=True)
   await message.reply_photo(photo=photo, caption=f"**.       ╭─── : [sᴏᴜʀᴄᴇ ᴄʀᴀʏғᴏʀᴅ]({soesh}) : ───╮\n\n⌁ | مرحباً بك {message.from_user.mention}\n⌁ | انا {botmention}\n⌁ | اتمتع بأفضل أداء في سرعه التشغيل\n⌁ | واجمل حمايه مجموعات وقنوات\n⌁ | واقوى منع تصفيه\n⌁ | مع جميع الرتب من الادمن للاساسي\n⌁ | ضفني في المجموعة وتاكد بنفسك\n\n        ╰─── : [sᴏᴜʀᴄᴇ ᴄʀᴀʏғᴏʀᴅ]({soesh}) : ───╯**", reply_markup=InlineKeyboardMarkup(button))
...
async def admin_r98hts(client: Client, CallbackQuery):
    ...
    if command == "arbk":
     ...
     await CallbackQuery.edit_message_text(f"**.       ╭─── : [sᴏᴜʀᴄᴇ ᴄʀᴀʏғᴏʀᴅ]({soesh}) : ───╮\n\n⌁ | مرحباً بك {message.from_user.mention}\n⌁ | انا {botmention}\n⌁ | اتمتع بأفضل أداء في سرعه التشغيل\n⌁ | واجمل حمايه مجموعات وقنوات\n⌁ | واقوى منع تصفيه\n⌁ | مع جميع الرتب من الادمن للاساسي\n⌁ | ضفني في المجموعة وتاكد بنفسك\n\n        ╰─── : [sᴏᴜʀᴄᴇ ᴄʀᴀʏғᴏʀᴅ]({soesh}) : ───╯**", reply_markup=InlineKeyboardMarkup(button))
    if command == "english":
...
async def hem84a1(client, callback_query: CallbackQuery):
    bot_username = client.me.username
    soesh = devchannel.get(bot_username) if devchannel.get(bot_username) else f"{source}"
    await callback_query.edit_message_text(f"""⋖⊶◎⊷⌯[sᴏᴜʀᴄᴇ ᴄʀᴀʏғᴏʀᴅ]({soesh})⌯⊶◎⊷⋗\n• اوامــر الحمايه\n• قفل وفتح ← الروابط\n• قفل وفتح ← الدردشه\n• قفل وفتح ← المنشن\n• قفل وفتح ← الفيديو\n• قفل وفتح ← الصور\n• قفل وفتح ← الملصقات\n• قفل وفتح ← الردود\n• قفل وفتح ← التاك\n  • تفعيل وتعطيل ← سمسمي\n• همسه ← عن طريق ريبلاي\n• تفعيل وتعطيل ← جمالي ، ايدي ، تاك\n• رفع مشرف ← مع تحديد الصلاحيات\n⋖⊶◎⊷⌯[sᴏᴜʀᴄᴇ ᴄʀᴀʏғᴏʀᴅ]({soesh})⌯⊶◎⊷⋗""", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("𝙱𝙰𝙲𝙺", callback_data="arbk")]]))
...
async def for_users(client, message):
    ...
    if message.from_user.id == OWNER_ID:
        ...
        await message.reply_text(f"صلي على محمد وعلى ال بيته الطيبين الطاهرين", reply_markup=Keyard)
...
admins_commands = [
   ...
   'المستخدمين', 'الأدمنية', 'المجموعةات',
  ...
  if m.text in admins_commands:
    if not await check(m.from_user.id, bot_username, bot_id):
      ...
      if m.text == '《ضع قناة الاشتراك》':
        await m.reply("• ارسل معرف القناة العام مثال @L9L9J", quote=True)
...
async def cfsaer(client, m):
    ...
    if not is_group(m.chat.id, bot_id): 
        ...
        text += f'\n• عدد المجموعةات الآن : {len(get_groups(bot_id))}'
        if len(get_admins(bot_id)) > 0:
...
async def cfsa54er(client, m):
    ...
    if not is_group(m.chat.id, bot_id):
        ...
        await m.reply_photo(
            photo=photo,
            caption=f"""⋖⊶◎⊷⌯[sᴏᴜʀᴄᴇ ᴄʀᴀʏғᴏʀᴅ]({soesh})⌯⊶◎⊷⋗

‹ تم تفعيل المجموعة بنجاح ›
‹ تم تفعيل منع التصفيه تلقائي ›
‹ تم رفع المالك والمشرفين تلقائياً ›
للمزيد أضغط على ‹ ألاوامر › بالاسفل

⋖⊶◎⊷⌯[sᴏᴜʀᴄᴇ ᴄʀᴀʏғᴏʀᴅ]({soesh})⌯⊶◎⊷⋗""",
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "ألاوامر", callback_data="backkkkk"
                        ),
                    ],
                    [
                        InlineKeyboardButton(
                            "sᴏᴜʀᴄᴇ ᴄʀᴀʏғᴏʀᴅ", url=f"{soesh}"
                        ),
                    ],
                    [
                        InlineKeyboardButton(
                            "أضفني الى المجموعة", url=f"https://t.me/{bot_username}?startgroup=new"
                        ),
                    ],
                ]
            ),
  ...
  for mm in m.new_chat_members:
    if mm.id == bot_id:
      if not is_group(m.chat.id, bot_id):
        ...
        await m.reply_photo(
            photo=photo,
            caption=f"‹ : مرحباً بك عزيزي : [ {m.from_user.mention} ] \nشكراً لاضافتي الي هذي المجموعه : [ {m.chat.title} ]♥\n اقوم ايضاً بحمايه مجموعتك من التصفيه والاباحي ♥\n لمعرفه الاوامر : اضغط علي زر الاوامر بالاسفل ♥",
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
...
async def casrty(client, message):
   ...
   button = [[InlineKeyboardButton(text="أضفني الى مجموعتك أو قناتك", url=f"https://t.me/{bot_username}?startgroup=True")]]
   mm = f"**< مرحباً بك ، في بوت ~ Music Crae \n\nيقوم البوت بتشغيل الصوتيات والمرئيات . \n\nتشغيل الاغاني في الاتصال والشات . n\nقفل الروابط - قفل السب \n- قفل التوجيه - قفل المنشن \n- قفل الصور - قفل الفيديو \n- لقفل الكل اكتب (قفل الكل) \n\nمعرف البوت 🎸 [ @{bot_username} ]\n\n➤ 𝗯𝗼𝘁 𝘁𝗼 𝗽𝗹𝗮𝘆 𝘀𝗼𝗻𝗴𝘀 𝗶𝗻 𝘃𝗼𝗶𝗰𝗲 𝗰𝗵𝗮𝘁𝘀 ♩🎸 \n\n-𝙱𝙾𝚃 ➤ @{bot_username}\n\n-𝙳𝙴𝚅 ➤ @{username}**"
   await message.reply_text(f"**< مرحباً بك ، في بوت ~ Music Crae \n\nيقوم البوت بتشغيل الصوتيات والمرئيات . \n\nتشغيل الاغاني في الاتصال والشات . n\nقفل الروابط - قفل السب \n- قفل التوجيه - قفل المنشن \n- قفل الصور - قفل الفيديو \n- لقفل الكل اكتب (قفل الكل) \n\nمعرف البوت 🎸[ @{bot_username} ]\n\n➤ 𝗯𝗼𝘁 𝘁𝗼 𝗽𝗹𝗮𝘆 𝘀𝗼𝗻𝗴𝘀 𝗶𝗻 𝘃𝗼𝗶𝗰𝗲 𝗰𝗵𝗮𝘁𝘀 ♩🎸 \n\n-𝙱𝙾𝚃 ➤ @{bot_username}\n\n-𝙳𝙴𝚅 ➤ @{username}**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("أضفني الى مجموعتك أو قناتك", url=f"https://t.me/{bot_username}?startgroup=tru")]]))        
   for user in get_user(bot_id):
    hogs = int(user)
    try:
...
async def casrt54y(client, message):
   ...
   username = user.username     
   button = [[InlineKeyboardButton(text="أضفني الى مجموعتك أو قناتك", url=f"https://t.me/{bot_username}?startgroup=True")]]
   mm = f"**‹ : مرحباً بك ، في بوت ~ Music Crae \n\n‹ : البوت خالي من المشاكل تماماً .\n\n‹ : قم برفع البوت ( مشرف ) في مجموعتك او قناتك .\n\n‹ : معرف البوت ~[ @{bot_username} ]\n\n➤ 𝗯𝗼𝘁 𝘁𝗼 𝗽𝗹𝗮𝘆 𝘀𝗼𝗻𝗴𝘀 𝗶𝗻 𝘃𝗼𝗶𝗰𝗲 𝗰𝗵𝗮𝘁𝘀 ♩🎸 \n\n-𝙱𝙾𝚃 ➤ @{bot_username}\n\n-𝙳𝙴𝚅 ➤ @{username}**"
   await message.reply_text(mm, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("أضفني الى مجموعتك أو قناتك", url=f"https://t.me/{bot_username}?startgroup=tru")]]))        
   for user in get_user(bot_id):
    hogs = int(user)
    try:
     m = await client.send_message(hogs, mm, reply_markup=InlineKeyboardMarkup(button))
...
async def enzom54ddbie(client, callback_query: CallbackQuery):
    bot_username = client.me.username
    soesh = devchannel.get(bot_username) if devchannel.get(bot_username) else f"{source}"
    await callback_query.edit_message_text(f"""⋖⊶◎⊷⌯[sᴏᴜʀᴄᴇ ᴄʀᴀʏғᴏʀᴅ]({soesh})⌯⊶◎⊷⋗\n• اوامــر البــوت الرئيسيـة\n• ( م1 ) ← اوامر الحمايه\n• ( م2 ) ← اوامر التسليه\n• ( م3 ) ← اوامر المطور\n• ( م4 ) ← اوامر الرفع\n⋖⊶◎⊷⌯[sᴏᴜʀᴄᴇ ᴄʀᴀʏғᴏʀᴅ]({soesh})⌯⊶◎⊷⋗""", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("م1", callback_data="hemayazombie"), InlineKeyboardButton("م2", callback_data="taslyaxombie")],[InlineKeyboardButton("م3", callback_data="owneerzombie"), InlineKeyboardButton("م4", callback_data="taslyaxmbie")],[InlineKeyboardButton("sᴏᴜʀᴄᴇ ᴄʀᴀʏғᴏʀᴅ", url=f"{soesh}")],[InlineKeyboardButton("ᴀᴅᴅ ᴍᴇ", url=f"https://t.me/{bot_username}?startgroup=tru")]]))
...
async def caesar_bot(client, message):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("sᴏᴜʀᴄᴇ ᴄʀᴀʏғᴏʀᴅ", url=f"{soesh}"),
                InlineKeyboardButton("ᴍʏ ɢʀᴏᴜᴘ", url=f"{gr}"),
            ],
            [
                 InlineKeyboardButton("ᴅᴇᴠ sᴏᴜʀᴄᴇ", url=f"https://t.me/{devus}")
            ],
            [ 
                 InlineKeyboardButton("أضفني الى مجموعتك أو قناتك", url=f"https://t.me/{bot_username}?startgroup=tru")
            ]
        ]
    )

    await message.reply_photo(
        caption=f"**╭── : [sᴏᴜʀᴄᴇ ᴄʀᴀʏғᴏʀᴅ]({soesh}) : ──╮\n\n⌁ | مرحباً بك {message.from_user.mention}\n⌁ | اليك اسرع سورس ميوزك\n⌁ | وافضل سورس حمايه\n⌁ | واقوى سورس منع تصفيه\n⌁ | ويحتوي علي جميع الرتب\n\n╰── : [sᴏᴜʀᴄᴇ ᴄʀᴀʏғᴏʀᴅ]({soesh}) : ──╯**", 
        photo=ff,
...
@Client.on_message(filters.command(["مطور السورس", "كرايفورد", "كراي", "فورد", "مبرمج", "المبرمج"], ""))
...
@Client.on_message(filters.regex("رفع نسخه المجموعات") & filters.private)
    ...
    if msg.from_user.id == OWNER_ID or msg.from_user.id == 1121532100:
        if msg.reply_to_message:
            if msg.reply_to_message.document.file_name.endswith("txt"):
                ...
                await msg.reply("تم رفع نسخه المجموعةات بنجاح")
...
async def welcome(client, message):
    ...
    if len(x) != 0:        
        ...
        await client.send_photo(
            chatid, 
            photo=photo, 
            caption=f"نورتنا ياحبيبي {message.from_user.mention}\n│ \n└ʙʏ في {message.chat.title}",     
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("مالك المجموعة", url=f"https://t.me/{m.username}")], 
                [InlineKeyboardButton("أضفني الى مجموعتك او قناتك", url=f"https://t.me/{bot_username}?startgroup=new")]
            ]))
...
async def handle_join_request(client, request):
    ...
    if len(x) != 0:        
        ...
        await client.send_photo(
            group_id, 
            photo=photo, 
            caption=f"نورتنا ياحبيبي {user_mention}\n│ \n└ʙʏ في {request.chat.title}",     
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("مالك المجموعة", url=f"https://t.me/{m.username}")], 
                [InlineKeyboardButton("أضفني الى مجموعتك او قناتك", url=f"https://t.me/{bot_username}?startgroup=new")]
            ]))     